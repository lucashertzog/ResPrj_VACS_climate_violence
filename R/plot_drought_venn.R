...
plot_drought_venn <- function(dat_imp){
  ...
  # Identify overlaps
  dat_imp$overlap <- apply(dat_imp[, .(slight_to_moderate, very_dry_drought_extreme, constant_drought_extreme, recent_long_period)], 1, function(x) {
    paste(names(x)[which(x == 1)], collapse = ", ")
  })
  
  # Create counts for each category combination
  overlap_counts <- data.frame(
    slight_to_moderate = dat_imp$slight_to_moderate,
  ...
  venn_data <- list(
    `Slight to Moderate` = which(dat_imp$slight_to_moderate == 1),
